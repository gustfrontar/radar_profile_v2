...
for my_radius in radius :
   for my_delta_z in delta_z :
       conf['delta_z'] = my_delta_z 
       conf['radius']  = my_radius

       print('Calculando para radio ',my_radius,' y dz ',my_delta_z)
       ...
       for ievent , my_event in enumerate( cases_dict['date'] ) :
          ...
          if len( conf['filelist'] ) > 0 :
             ...
             pickle_file = output_path + '/event_profiles_' + cases_dict['ID'][ievent] + '_' + dt.strftime( cases_dict['date'][ievent] , '%Y%m%d%H' ) + '_sens_r' + str(my_radius) + '_dz_' + str(my_delta_z) + '.pkl'
